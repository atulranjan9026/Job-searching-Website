...
return `data:image/png;base64,${btoa(String.fromCharCode(...new Uint8Array(buffer)))}`
}
...
function KNNApp() {
  ...
  const handleKNN = async () => {
    ...
    const {mostCommonClass, distances} = knn(dataknn, queryPoint, k, setDistances);
  ...
  return (
    ...
    <h2 className='title'>WAGER</h2>
      ...
      <br />
      <div>
        <label>Query Point:</label> <br />
        <input type="text" value={queryPoint.join(",")} />
      </div>{" "}
      <br />
      <button onClick={handleKNN}>Run KNN</button>
      <div className='cards'>
      {idData.map((user)=>(
        <div key={user.name}>        
        <div className="card">
        ...
        <div className="tourDetails">
                ...
                <button className="btnRed" onClick={() => { historyCard.push("/Search") }} >Book
            </button>
                </div>            
        </div>))}
      </div>
